...
from watermark_anything.data.metrics import (
    accuracy,
    bit_accuracy,
    bit_accuracy_inference,
    iou,
    psnr,
)
...
from watermark_anything.utils.image import (
    create_diff_img,
    create_fixed_color_palette,
    detect_wm_hm,
    masks_to_colored_image,
)
...
def get_parser():
    ...
    aa(
        "--attenuation_config",
        type=str,
        default="configs/attenuation.yaml",
    ...
    aa(
        ...
        default=None,
        type=str,
        help="What parts of the embedder to freeze",
    )
    ...
    aa(
        ...
        type=utils.bool_inst,
        default=False,
        help="If True, only runs evaluate",
    )
    ...
    aa(
        ...
        type=float,
        default=0,
        help="probability to inpaint betweem images of each batch.",
    )
    aa(
        "--multiple_w",
        type=float,
        default=0,
        help="probability to use 2 watermarks instead of 1.",
    )
    aa(
        "--eval_multiple_w",
        type=utils.bool_inst,
        default=False,
        help="evaluate with multiple watermarks.",
    )
    aa(
        "--nb_wm_eval",
        type=int,
        default=5,
        help="how many watermarks to use for evaluation (default: 5)",
    )
    aa("--nb_images_eval", default=1000, type=int, help="Number of images to evaluate")
    aa(
        "--nb_images_eval_multiple_w",
        default=1000,
        type=int,
        help="Number of images to evaluate for multiple wm. Takes longer.",
    )
...
def main(params):
    ...
    if params.multiple_w > 0:
        print(
            "Training with multiple watermarks. Adding multiple watermark evaluation."
        )
    ...
    wam = Wam(
        ...
        params.scaling_w,
        params.scaling_i,
        roll_probability=params.roll_probability,
    )
    wam.to(device)
    ...
    image_detection_loss = LPIPSWithDiscriminator(
        ...
        detect_weight=params.lambda_w,
        decode_weight=params.lambda_w2,
    ...
    optim_params_d = (
        ...
        else uoptim.parse_params(params.optimizer_d)
    )
    ...
    train_transform, train_mask_transform, val_transform, val_mask_transform = (
        get_transforms_segmentation(params.img_size)
    )

    if "COCO" in params.train_dir:
        train_loader = get_dataloader_segmentation(
            params.train_dir,
            params.train_annotation_file,
            transform=train_transform,
            mask_transform=train_mask_transform,
            batch_size=params.batch_size,
            num_workers=params.workers,
            shuffle=False,
            multi_w=params.multiple_w > 0,
        )
        val_loader = get_dataloader_segmentation(
            params.val_dir,
            params.val_annotation_file,
        ...
        val_loader_multi_wm = get_dataloader_segmentation(
            ...
            random_nb_object=False,
            multi_w=True,
            max_nb_masks=params.nb_wm_eval,
        )
    else:
    ...
    _, individuals = augmenter.mask_embedder.sample_multiwm_masks(
        dummy_img, nb_times=params.nb_wm_eval
    )  # nb_wm_eval, 1, 256, 256
    # Save validation masks if in the main process
    if udist.is_main_process():
        save_image(
            validation_masks, os.path.join(params.output_dir, "validation_masks.png")
        )
        save_image(
            individuals, os.path.join(params.output_dir, "validation_masks_multiwm.png")
        )
    ...
    if params.only_eval:
        ...
        if udist.is_main_process():
            ...
            if params.eval_multiple_w:
                with open(
                    os.path.join(params.output_dir, "log_only_eval_kwm.txt"), "a"
                ) as f:
    ...
    for epoch in range(start_epoch, params.epochs):
        ...
        if epoch % params.eval_freq == 0:
            ...
            log_stats = {
                ...
                **{f"val_kwm_{k}": v for k, v in val_stats_kwm.items()},
            }
        ...
        udist.save_on_master(
            save_dict, os.path.join(params.output_dir, "checkpoint.pth")
        )
        if params.saveckpt_freq and epoch % params.saveckpt_freq == 0:
            udist.save_on_master(
                save_dict, os.path.join(params.output_dir, f"checkpoint{epoch:03}.pth")
            )
...
def train_one_epoch(
    ...
    resize = transforms.Resize(
        (h, w), interpolation=transforms.InterpolationMode.BILINEAR, antialias=True
    )

    # Iterate over the training data loader
    for it, (imgs, masks) in enumerate(
        metric_logger.log_every(train_loader, 10, header)
    ):
        # Move images to the appropriate device
        imgs = imgs.to(device, non_blocking=True)

        # Forward pass through the model
        outputs, wam_time = timed(
            lambda: wam(imgs, masks, no_overlap=params.multiple_w > 0, params=params)
        )
        print("time", wam_time)
        ...
        last_layer = (
            wam.embedder.get_last_layer()
            if not params.distributed
            else wam.module.embedder.get_last_layer()
        )
        ...
        if params.nbits > 0:
            bit_accuracy_ = (
                bit_accuracy(
                    outputs["preds"][:, 1:, :, :], outputs["msgs"], outputs["masks"]
                )
                .nanmean()
                .item()
            )
        ...
        for method, mask in zip([""], [mask_preds]):
            log_stats.update(
                {
                    f"acc{method}": accuracy(
                        mask, outputs["masks"].max(1).values.float().unsqueeze(1)
                    )
                    .mean()
                    .item(),
                    f"iou_0{method}": iou(
                        mask,
                        outputs["masks"].max(1).values.float().unsqueeze(1),
                        label=0,
                    )
                    .mean()
                    .item(),
            ...
            log_stats[f"miou{method}"] = (
                log_stats[f"iou_0{method}"] + log_stats[f"iou_1{method}"]
            ) / 2
        ...
        colored_masks = masks_to_colored_image(
            outputs["masks"].float(), color_palette
        )  # b 3 h w

        if (
            epoch % params.saveimg_freq == 0
            and it % 200 == 0
            and udist.is_main_process()
        ):
            # Save original, watermarked, and augmented images, as well as differences and masks
            save_image(
                unnormalize_img(imgs),
                os.path.join(params.output_dir, f"{epoch:03}_{it:03}_train_0_ori.png"),
                nrow=8,
            )
            save_image(
                unnormalize_img(outputs["imgs_w"]),
                os.path.join(params.output_dir, f"{epoch:03}_{it:03}_train_1_w.png"),
                nrow=8,
            )
            save_image(
                create_diff_img(imgs, outputs["imgs_w"]),
                os.path.join(params.output_dir, f"{epoch:03}_{it:03}_train_2_diff.png"),
                nrow=8,
            )
            save_image(
                unnormalize_img(outputs["imgs_aug"]),
                os.path.join(params.output_dir, f"{epoch:03}_{it:03}_train_3_aug.png"),
                nrow=8,
            )
            # Save predicted and target masks
            save_image(
                colored_masks.float(),
                os.path.join(params.output_dir, f"{epoch:03}_{it:03}_train_4_mask.png"),
                nrow=8,
            )
            save_image(
                F.sigmoid(mask_preds / params.temperature),
                os.path.join(params.output_dir, f"{epoch:03}_{it:03}_train_5_pred.png"),
                nrow=8,
            )
...
def eval_full(
    ...
    for it, (imgs, masks) in enumerate(metric_logger.log_every(val_loader, 10, header)):
        ...
        inverse_resize_mask = transforms.Resize(
        ...
        if it * params.batch_size_eval >= params.nb_images_eval:
            break
        ...
        for mask_id, masks in enumerate(validation_masks_and_seg):
            ...
            for transform, strengths in validation_augs:
                ...
                for strength in strengths:
                    ...
                    imgs_aug_ori, masks_aug_ori = inverse_resize(
                        imgs_aug
                    ), inverse_resize_mask(masks_aug)

                    # Resize augmented images if necessary
                    if imgs_aug.shape[-2:] != (h, w):
                        imgs_aug = nn.functional.interpolate(
                            imgs_aug,
                            size=(h, w),
                            mode="bilinear",
                            align_corners=False,
                        ...
                        masks_aug = nn.functional.interpolate(
                            masks_aug, size=(h, w), mode="nearest"
                        )
                    ...
                    if params.nbits > 0:
                        bit_preds = preds[:, 1:, :, :]
                        bit_accuracy_ = (
                            bit_accuracy(bit_preds, msgs.unsqueeze(1), masks_aug)
                            .nanmean()
                            .item()
                        )
                    ...
                    for method, mask_preds_ in [("", mask_preds)]:
                        ...
                        log_stats.update(
                            {
                                f"acc{method}": accuracy(mask_preds_, masks_aug)
                                .mean()
                                .item(),
                                f"iou_0{method}": iou(mask_preds_, masks_aug, label=0)
                                .mean()
                                .item(),
                                f"iou_1{method}": iou(mask_preds_, masks_aug, label=1)
                                .mean()
                                .item(),
                                f"avg_pred{method}": torch.sigmoid(mask_preds_)
                                .mean()
                                .item(),
                                f"avg_pred_hard_{method}": (
                                    torch.sigmoid(mask_preds) > 0.5
                                )
                                .float()
                                .mean()
                                .item(),
                                f"avg_target{method}": masks_aug.mean().item(),
                                f"norm_avg{method}": torch.norm(
                                    torch.sigmoid(mask_preds_), p=2
                                ).item(),
                            }
                        )
                        # Calculate mean IoU
                        ...
                        if params.nbits > 0:
                            for decode_method in ["semihard", "soft"]:
                                log_stats[f"bit_acc{method}_{decode_method}"] = (
                                    bit_accuracy_inference(
                                        bit_preds,
                                        msgs,
                    ...
                    if (
                        epoch % params.saveimg_freq == 0 or params.only_eval
                    ) and udist.is_main_process():
                        if current_key in tosave:
                            ...
                            imgs_tosave.append(
                                inverse_resize(
                                    F.sigmoid(mask_preds[idx]).repeat(3, 1, 1)
                                ).cpu()
                            )
    ...
    if (
        ...
        save_image(
            torch.stack(imgs_tosave),
            os.path.join(params.output_dir, f"{epoch:03}_val_{eval_name}{aux}.png"),
            nrow=5,
        )
    # Synchronize metric logger across processes
    metric_logger.synchronize_between_processes()
...
def eval_full_kwm(
    ...
    imgs_tosave = {
        ...
        "rectangles": {True: [], False: []},
    }

    # Iterate over the validation data loader
    for it, (imgs, masks) in enumerate(metric_logger.log_every(val_loader, 10, header)):
        # Break if the number of evaluated images exceeds the limit
        if it * params.batch_size_eval >= params.nb_images_eval_multiple_w:
            break

        # Iterate over roll options (True/False)
        for roll in [True, False]:
            # Iterate over mask types
            for name_mask, masks_ in [
                ("segmentation", masks),
                ("rectangles", validation_masks),
            ]:
                ...
                for nb_wm in range(nb_wm_eval):
                    ...
                    if params.nbits > 0:
                        ...
                        bit_accuracy_ = (
                            bit_accuracy(
                                bit_preds,
                                (
                                    msgs_l[0].unsqueeze(1)
                                    if not roll
                                    else torch.roll(
                                        msgs_l[0].unsqueeze(1), shifts=-1, dims=0
                                    )
                                ),
                                (
                                    first_mask
                                    if not roll
                                    else torch.roll(first_mask, shifts=-1, dims=0)
                                ),
                            )
                            .nanmean()
                            .item()
                        ...
                        bit_preds = preds[:, 1:, :, :]
                        bit_accuracy_cummulate = (
                            bit_accuracy(
                                bit_preds,
                                (
                                    msgs_l_aux
                                    if not roll
                                    else torch.roll(msgs_l_aux, shifts=-1, dims=0)
                                ),
                                (
                                    masks_[:, : nb_wm + 1, :, :]
                                    if not roll
                                    else torch.roll(
                                        masks_[:, : nb_wm + 1, :, :], shifts=-1, dims=0
                                    )
                                ),
                            )
                            .nanmean()
                        ...
                        log_stats[f"bit_acc_{nb_wm_name}_{name_mask}_roll={roll}"] = (
                            bit_accuracy_
                        )
                        log_stats[
                            f"bit_acc_cummulate_{nb_wm_name}_{name_mask}_roll={roll}"
                        ] = bit_accuracy_cummulate

                    # Compute stats for the augmentation and strength
                    for mask_preds_, mask_target in [
                        (
                            mask_preds,
                            (
                                combined_mask
                                if not roll
                                else torch.roll(combined_mask, shifts=-1, dims=0)
                            ),
                        )
                    ]:
                        ...
                        log_stats.update(
                            {
                                f"acc_{nb_wm_name}_{name_mask}_roll={roll}": accuracy(
                                    mask_preds_, mask_target
                                )
                                .mean()
                                .item(),
                                f"iou_0_{nb_wm_name}_{name_mask}_roll={roll}": iou(
                                    mask_preds_, mask_target, label=0
                                )
                                .mean()
                                .item(),
                                f"iou_1_{nb_wm_name}_{name_mask}_roll={roll}": iou(
                                    mask_preds_, mask_target, label=1
                                )
                                .mean()
                                .item(),
                                ...
                                f"norm_avg_{nb_wm_name}_{name_mask}_roll={roll}": torch.norm(
                                    mask_preds_, p=2
                                ).item(),
                            }
                        )
                    ...
                    if (
                        ...
                        idx = 1
                        imgs_tosave[name_mask][roll].append(
                            unnormalize_img(imgs[idx].cpu())
                        )
                        imgs_tosave[name_mask][roll].append(
                            unnormalize_img(imgs_w[idx if not roll else idx + 1].cpu())
                        )
                        imgs_tosave[name_mask][roll].append(
                            unnormalize_img(combined_imgs[idx].cpu())
                        )
                        imgs_tosave[name_mask][roll].append(
                            mask[idx if not roll else idx + 1].cpu().repeat(3, 1, 1)
                        )
                        imgs_tosave[name_mask][roll].append(
                            combined_mask[idx if not roll else idx + 1]
                            .cpu()
                            .repeat(3, 1, 1)
                        )
                        imgs_tosave[name_mask][roll].append(
    ...
    if (
        ...
        aux = (
            ...
            else f"_only_eval_{nb_wm_eval}_wm"
        )
        for roll in [False, True]:
            save_image(
                torch.stack(imgs_tosave["segmentation"][roll]),
                os.path.join(
                    params.output_dir,
                    f"{epoch:03}_val_full{aux}_segmentation_roll={roll}.png",
                ),
                nrow=6,
            )
            save_image(
                torch.stack(imgs_tosave["rectangles"][roll]),
                os.path.join(
                    params.output_dir,
                    f"{epoch:03}_val_full{aux}_rectangles_roll={roll}.png",
                ),
                nrow=6,
            )
